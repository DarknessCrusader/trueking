...
# sed -i "s|VPC_ID|$VPC_ID|g" /home/ec2-user/k8s/cluster.yaml
# sed -i "s|PRIVATE_SUB_A|$PRIVATE_SUB_A|g" /home/ec2-user/k8s/cluster.yaml
# sed -i "s|PRIVATE_SUB_B|$PRIVATE_SUB_B|g" /home/ec2-user/k8s/cluster.yaml
# sed -i "s|EKS_KMS|$EKS_KMS|g" /home/ec2-user/k8s/cluster.yaml

# eksctl create cluster -f /home/ec2-user/k8s/cluster.yaml

# # --- 4. EKS SG 허용 (CloudShell + VPC CIDR) ---
# CLUSTER_SG=$(aws eks describe-cluster --name wsc2026-eks-cluster --query "cluster.resourcesVpcConfig.clusterSecurityGroupId" --output text)
# CLOUDSHELL_SG=$(aws ec2 describe-security-groups --filters Name=group-name,Values=mark-sg --query "SecurityGroups[0].GroupId" --output text)
...
# aws eks update-kubeconfig --name wsc2026-eks-cluster --region $REGION
# source kubectl-connect wsc2026-eks-cluster
# # Access Entry for current user
# aws eks create-access-entry --cluster-name wsc2026-eks-cluster --principal-arn arn:aws:iam::${ACCOUNT}:root --type STANDARD 2>/dev/null
# aws eks associate-access-policy --cluster-name wsc2026-eks-cluster --principal-arn arn:aws:iam::${ACCOUNT}:root --policy-arn arn:aws:eks::aws:cluster-access-policy/AmazonEKSClusterAdminPolicy --access-scope type=cluster 2>/dev/null
