...
# Wait for Pod Identity Agent to be running on all nodes
echo "Waiting for Pod Identity Agent pods..."
for i in $(seq 1 30); do
  READY=$(kubectl get ds -n kube-system -l app.kubernetes.io/name=eks-pod-identity-agent -o jsonpath='{.items[0].status.numberReady}' 2>/dev/null)
  DESIRED=$(kubectl get ds -n kube-system -l app.kubernetes.io/name=eks-pod-identity-agent -o jsonpath='{.items[0].status.desiredNumberScheduled}' 2>/dev/null)
  if [ -n "$READY" ] && [ "$READY" = "$DESIRED" ] && [ "$READY" -ge 1 ]; then echo "Pod Identity Agent ready ($READY/$DESIRED)"; break; fi
  sleep 10
done
...
# Pod Identity 확실히 적용되도록 restart
sleep 10
...
# AllowAssumeAuditRole for marking IAM user
aws iam put-user-policy --user-name $MARK_USER --policy-name AllowAssumeAuditRole \
  --policy-document "{\"Version\":\"2012-10-17\",\"Statement\":[{\"Effect\":\"Allow\",\"Action\":\"sts:AssumeRole\",\"Resource\":\"arn:aws:iam::${ACCOUNT}:role/unicorn-audit-role\"}]}" || true
