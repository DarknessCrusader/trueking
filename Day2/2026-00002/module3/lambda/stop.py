...
def handler(event, context):
    ...
    message = {
        "event": "EC2_STOPPED",
        "timestamp": now(),
        "detail": f"Instance {INSTANCE_ID} restarted, SG {SECURITY_GROUP_ID} cleaned",
        "action": "RESTORED",
    }
    sns.publish(TopicArn=SNS_TOPIC_ARN, Subject="EC2 stop remediation", Message=json.dumps(message))
